...
- **Automation safety**: the isolated-instance launcher uses a separate Electron `--user-data-dir` and a direct binary launch on a random free port (9300-9399), so it runs alongside the user's live Obsidian without conflict. Isolated instances are the expected way to test PRs and reproduce bugs even when the user's live Obsidian is open. The launcher does fail fast if the same `--vault` is already in use (another isolated instance with the same vault dir), or if the requested debugger port is already occupied - in both cases re-run with a different `--vault` name or port rather than assuming live Obsidian is the blocker.
