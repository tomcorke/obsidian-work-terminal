...
- **Preview in Work Terminal panel** - render the task file's markdown read-only inside the Work Terminal panel as a "Preview" pseudo-tab next to Shell and agent tabs. Selecting the Preview tab shows the markdown; selecting any other tab hides the preview and shows the selected tab's content, so terminals and agent sessions stay reachable while the preview is available. A task selection automatically focuses the Preview tab, matching the Embedded placement's behaviour. Provides an **Open in editor** button that opens the file in a workspace leaf using the same target-leaf resolution as the Navigate placement; clicking it does not dismiss the Preview tab. The preview re-renders automatically when the file is modified in another leaf, so edits you make elsewhere show up immediately. Best for small screens or single-pane layouts where keeping the terminal accessible while glancing at the task description matters more than editing in place. There is intentionally no way to edit the file from the preview - use **Open in editor** for that.
...
**Context prompt placement**: **Append context prompt automatically** is enabled by default and preserves the agent type's positional or flag-based injection behavior. Disable it when a CLI requires explicit placement, then include `$workTerminalPrompt` in **Arguments** (for example, `--prompt $workTerminalPrompt`); saving and launching are blocked if the placeholder is missing. **Escape $workTerminalPrompt as one argument** is enabled by default in manual mode so multiline prompts containing spaces, quotes, or backslashes remain one argv value, including through login-shell wrapping. Disable escaping only when deliberate raw substitution and argument splitting are required. Both options are included in profile import/export, and the resolved launch preview shows which mode is active.
